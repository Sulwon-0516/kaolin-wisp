...
def parse_options(return_parser=False):
    ...
    net_group.add_argument('--deform-org', type=bool, default=True,
                          help='Use original deformation net')
    net_group.add_argument('--slice-org', type=bool, default=True,
                          help='Use original slice net')
    net_group.add_argument('--template-org', type=bool, default=False,
                          help='Use oroginal template net')
    net_group.add_argument('--high-dim-grid', type=bool, default=False,
                          help='Use 3+amb dimension hash grid')
...
def get_modules_from_config(args):
    ...
    if args.nef_type == "HyperNeuralRadianceField":
        transform = HyperSampleRays(args.num_rays_sampled_per_img)
        train_dataset = MultiviewDatasetHyper(**vars(args), transform=transform)
        train_dataset.init()
            ...
            if isinstance(pipeline.nef.grid, HashGrid):
                if not args.valid_only:
                    if args.tree_type == 'quad':
                        ...
                        pipeline.nef.deform_grid.init_from_octree(args.base_lod, args.num_lods)

                        if pipeline.nef.use_seperate_grid:
                            pipeline.nef.surface_grid.init_from_octree(args.base_lod, args.num_lods)
                    elif args.tree_type == 'geometric':
                        pipeline.nef.grid.init_from_geometric(16, args.max_grid_res, args.num_lods)
                        pipeline.nef.deform_grid.init_from_geometric(16, args.max_grid_res, args.num_lods)
                        if pipeline.nef.use_seperate_grid:
                            pipeline.nef.surface_grid.init_from_geometric(16, args.max_grid_res, args.num_lods)
                    else:
