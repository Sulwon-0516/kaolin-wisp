...
at::Tensor hashgrid_interpolate_cuda_hyper(
    ...
    for (int32_t i=0; i < resolution.size(); ++i) {
        hashgrid_interpolate_cuda_hyper_impl(num_coords, codebook_size, feature_dim, resolution[i], i, num_lods, coords, codebook[i], feats);
    }
    return feats;
#else
    AT_ERROR(__func__);
...
std::vector<at::Tensor> hashgrid_interpolate_backward_cuda_hyper(
    ...
    for (int32_t i=0; i < resolution.size(); ++i) {
        hashgrid_interpolate_backward_cuda_hyper_impl(num_coords, codebook_size, feature_dim, resolution[i], i, num_lods, 
                coords, grad_output, grad_codebook[i]);
    }
    return grad_codebook;
#else
